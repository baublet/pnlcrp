 ...
 *
 * $Id: view_photo.php 17958 2008-09-02 15:34:53Z JensT $
 */
...
<link href="../strikestyles.css" rel="stylesheet" type="text/css">
  ...
  <div><img src="/labhist/strike/headerimages/strike-logo.jpg" width="829" 
height="152"
alt="The 1919 Seattle General Strike"></div>
...
if($useIcons && sizeof($albumItemOptions) > 2) {
...
$gallery->app->emailOn == "yes")
{
	...
	if (! $gallery->album->getEmailMe('comments', $gallery->user)) {
		...
		$url= makeAlbumUrl(
			...
			$id,
			array('emailMeComments' => ($gallery->album->getEmailMe('comments', $gallery->user, $id)) ? 'false' : 'true')
		);
